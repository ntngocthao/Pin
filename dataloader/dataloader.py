...
class DF():
    ...
    def read_one_csv(self,file_name,nominal_capacity=None):
        ...
        if nominal_capacity is not None:
            ...
            # pandas moi (>=2.x) khong con tu dong ep kieu int64 -> float64 khi gan
            # bang .iloc, nen phai noi lai DataFrame thay vi gan truc tiep.
            # (newer pandas no longer silently casts int64 -> float64 on .iloc
            # assignment, so rebuild the DataFrame instead of assigning in place)
            df = pd.concat([f_df, df.iloc[:, -1:]], axis=1)
